...
## 待办：用户手册
- 写作时机：数据覆盖基本完整后（目标：11家公司MCU数据均有至少2022-2024）
- 文件位置：docs/USER_GUIDE.md
- 必须包含"各公司MCU数据来源说明"章节，逐家说明：
  - 数据口径（年报哪张表/哪一行）
  - 获取方式（PDF提取 / AKShare推算 / 手工录入）
  - 置信度与局限性（如002180只能用极海子公司「芯片」产品线，无纯MCU拆分）
  - 历史数据断层（如300327仅2023起、688766仅2022起）
  - 更新方法（每季度跑哪个脚本）
- 参考数据源：mcu_known_data.json 的 source 字段 + companies_meta.json 的 mcu_note 字段
- 受众：接手维护的同事，需能独立完成季度更新

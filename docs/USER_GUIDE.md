...
| 🔄 代码已支持 | 688380 中微半导、688279 峰岹、688595 芯海、688391 钜泉、688018 乐鑫 | `fetch_mcu_data.py` 已从利润表 `OPERATE_COST` 计算**总公司毛利率**；在本地/Cloud Shell 运行后自动填充（精度因公司而异，见各公司说明） |

> **精度说明**：688380（MCU≈99%）精度最高，与MCU毛利率几乎等同；688018（芯片≈85%）次之；688279/688595/688391 含非MCU产品，为粗略代理。如需精确分产品毛利率，仍需运行 `extract_mcu_segments.py` 提取年报PDF。
...
| **毛利率覆盖** | 🔄 总公司毛利率（运行 `fetch_mcu_data.py` 后自动填充 2022–2025 上市后年份）；MCU≈99%，**精度极高**，与MCU毛利率误差<1% |
| **局限性** | 2018–2021营收来自招股书，为上市前数据；毛利率仅覆盖上市后年份 |
| **更新方法** | `python fetch_mcu_data.py 688380`（AKShare自动拉取总收入+营业成本） |
...
| **毛利率覆盖** | 🔄 总公司毛利率（运行 `fetch_mcu_data.py` 后自动填充 2021–2025 上市后年份）；MCU≈67%，含HVIC/MOSFET等非MCU产品，偏差约10-15% |
| **局限性** | 系数0.67为静态估算，实际MCU占比随产品结构变化；年报未拆分MCU vs ASIC vs HVIC；毛利率为总公司代理值 |
...
| **毛利率覆盖** | 🔄 总公司毛利率（运行 `fetch_mcu_data.py` 后自动填充 2020–2025 上市后年份）；MCU约占总收入45%，含模拟信号链等非MCU产品，为粗略代理 |
| **局限性** | 年报同时披露「MCU和AIoT芯片合计」和「MCU芯片单行」——本库使用单行MCU口径；毛利率为总公司代理值；如需MCU分产品毛利率需PDF提取 |
| **更新方法** | MCU收入：`python extract_mcu_segments.py 688595`；毛利率：`python fetch_mcu_data.py 688595` |
...
| **MCU历史覆盖** | 2018–2025（8年完整，2018–2020来自招股书） |
| **员工数覆盖** | 2022–2025（上市后年报），2018–2020无（招股书未提取） |
| **毛利率覆盖** | 🔄 总公司毛利率（运行 `fetch_mcu_data.py` 后自动填充 2021–2025 上市后年份）；MCU/计量SoC约占总收入34%，为粗略代理，代理误差相对较大 |
| **局限性** | 系数0.34为静态估算，应用场景单一（智能电表计量），实际占比相对稳定；毛利率为总公司代理值 |
...
| **毛利率覆盖** | 🔄 总公司毛利率（运行 `fetch_mcu_data.py` 后自动填充 2019–2025 上市后年份）；芯片收入约占总收入85%，为**较优代理** |
| **局限性** | 「芯片」行含Wi-Fi/BT SoC和RISC-V MCU，属于广义MCU口径；2018年无GCS PDF；模组收入约占15%，对总毛利率有轻微稀释 |
| **更新方法** | MCU收入：`python extract_mcu_segments.py 688018`；毛利率：`python fetch_mcu_data.py 688018` |
...
# Step 6: 更新AKShare数据（需能访问eastmoney.com，建议在Cloud Shell/本地跑）
# 同时拉取：总营收、净利润、研发费用、营业成本（→自动计算毛利率）、员工数
...
Claude Code远端环境无法访问AKShare（东方财富 API 返回 403），运行fetch_mcu_data.py会**清空data.json的financials字段**。
- 规避方法：用`git checkout HEAD -- data.json`还原，再用Python脚本手动patch字段
- 不要在Claude Code session中运行`python fetch_mcu_data.py`
- **毛利率填充**：代码已支持从 `OPERATE_COST` 自动计算毛利率，但需在本地/Cloud Shell（可访问 eastmoney.com）运行才能实际写入数据
